  ...
  override class func spec() {
    ...
    describe("detect") {
      it("detects") {
        ...
        zeroData.withUnsafeBytes { bytes in
          let zero = bytes.baseAddress!.assumingMemoryBound(to: Int16.self)
          expect(detector.detect(frames: zero, count: 80)) == .inActiveVoice
          expect(detector.detect(frames: zero, lengthInMilliSec: 10)) == .inActiveVoice
        }
      }
      ...
      it("should fail with inacceptable frame count") {
        ...
        zeroData.withUnsafeBytes { bytes in
          let zero = bytes.baseAddress!.assumingMemoryBound(to: Int16.self)
          expect { _ = detector.detect(frames: zero, count: 81) }.to(throwAssertion())
        }
      }
      #endif
    }
    ...
    describe("detect(sampleBuffer, ...)") {
      it("does") {
        ...
        let url = Bundle.module.url(forResource: "3722", withExtension: "mp3")!
        let reader = try! AudioTrackReader(audioURL: url, timeRange: nil, settings: settings)
